...
const LandlordLeaseAgreement = () => {
  ...
  // Placeholder: In a real app, fetch the lease document URL from the backend using the listingId
  const leaseDocumentUrl = null; // e.g., '/api/leases/{listingId}/document.pdf'
  ...
  useEffect(() => {
    // Test with listing id 10 for debugging
    const testListingId = 10;
    setLeaseLoading(true);
    const fetchLease = async () => {
      // Hardcode listing_id 10 for debugging
      const testListingId = 10;
      const { data, error } = await supabase
        ...
        .limit(1)
        .single();
      console.log('Lease query result:', { data, error, listingId });
      if (data && !error) {
        setLease(data);
      } else {
        setLease(null);
    ...
    fetchLease();
  }, [listingId]); // Only run once for the test
        ...
        <div className="w-full max-w-xl bg-white rounded shadow p-6 mb-8 mx-auto">
          ...
          {leaseLoading ? (
            ...
            <div className="space-y-2 text-center">
              ...
              {/* Full-width Send New Lease button at the bottom of Lease Details */}
              <Button
                className="w-full mt-8 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded shadow"
                onClick={() => {
                  // Redirect to landlord matches page
                  navigate('/landlord/matches'); //TODO: change to redirect to correct page (landlord matches)
                }}
              >
                Send New Lease
              </Button>
...
export default LandlordLeaseAgreement; 